...
- **发送机制**：存在异常（0x01~0x04）时以 **50ms** 周期持续发送；故障消除时连续补发 **10次（50ms周期）** 清除确认帧；常态全正常时以 **100ms** 周期持续心跳发送；本地 MOS 状态跳变时立即触发发送。
...
| **Byte 2** | **电池 1 主放电 MOS 状态** | `0x00`: 关闭 (OFF)<br>`0x01`: 开启 (ON) (对应 PA0 引脚实际电平) |
