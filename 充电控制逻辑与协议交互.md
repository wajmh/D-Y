...
flowchart TD
    ...
    ApplyMos --> TxReply["距上次 >= 200ms 或 目标电池切换?<br>FDCAN_SendChargeReplyToCharger 代理发送"]
```
...
代码位置：[`Core/Src/fdcan.c`](file:///home/yq/ghf/workspace/D-Y-A10/Core/Src/fdcan.c#L849-L924)

充电桩并不直接与某单块电池物理直连通信，而是由 STM32 充当协议网关：
1. **日常缓存机制**：
   在后台接收电池 BMS 报文时，`FDCAN_CacheChargeReplyFrame()` 会把两块电池的 3 种基础报文实时缓存至 `batteryChargeReplyFrames[2][3]`：
   - **Slot 0**：电池总压/总电流状态帧（原始 ID：`0x04028000U`）
   - **Slot 1**：电池容量与基本信息帧（原始 ID：`0x04008000U`）
   - **Slot 2**：电芯温度状态帧（原始 ID：`0x04088000U`）
2. **报文 ID 伪装映射**：
   当需要向充电桩发送数据时，调用 [`FDCAN_SendChargeReplyToCharger()`](file:///home/yq/ghf/workspace/D-Y-A10/Core/Src/fdcan.c#L849-L924)：
   ```c
   ...
   - 通过 `hfdcan3`（因硬件反接软件对调后由 FDCAN3 连接外部充电桩）发送给外部充电桩。
3. **发送触发频次**：
